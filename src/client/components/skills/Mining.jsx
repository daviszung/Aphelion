    ...
    <div className='woodcuttingSkillContainer'>
      <div className='skillInfoContainer miningColor'>
        <div className='innerInfoContainer'>
          ...
          <div className='flexAlign'><div style={{marginRight: '10px'}}>CURRENT AXE</div><div className='axeType'>{pickaxeType[state.modifiers.pickaxe]}</div></div>
        </div>
      ...
      <button id='normalLogBtn' className='actionBtn miningColor' onClick={() => {
        if (selectedAction === 'Normal Log') {
          setSelectedAction(null)
        } else {setSelectedAction('Normal Log')}}}>
        ...
        <div className='progressBarContainer'>
          <div className='progressBar' style={selectedAction === 'Normal Log' ? {'animation': `load ${3 * ((100 - state.modifiers.pickaxe) / 100)}s linear infinite`} : {width: '0%'}}></div>
        </div>
      </button>
    </div>
  )
